...
| Requirement | Implementation | Evidence |
|-------------|----------------|----------|
| Changes are authorized and tested before release | PRs to `main` run the lint workflow (yamllint, hadolint, actionlint, zizmor); integration suite `make test-waf` (20 go-ftw cases) validates WAF behavior; container scans gate CRITICAL/HIGH before push | lint.yml:4-5,41-52; Makefile:103-104; tests/integration/baseline.yaml; tests/integration/owasp-crs-core.yaml; tests/integration/owasp-crs-bypass.yaml; .github/workflows/docker-build-scan-sign.yml:100-115 |
